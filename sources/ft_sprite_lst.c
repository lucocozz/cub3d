...
/*   Updated: 2021/02/16 14:55:26 by lucocozz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_sprite_lst	*ft_create_sprite(t_garbage *garb, t_coord axe, int texture)
{
	...
	sprite->img.data = (int*)mlx_get_data_addr(sprite->img.ptr,
		&sprite->img.bpp, &sprite->img.size, &sprite->img.endian);
	sprite->coord.x = axe.x + 0.5;
	sprite->coord.y = axe.y + 0.5;
	sprite->next = NULL;
	return (sprite);
}

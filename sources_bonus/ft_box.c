...
/*   Updated: 2020/06/01 15:29:22 by lucocozz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// void	ft_display_floor(t_parsing parse, t_box *box, t_mlx *mlx)
// {
...
void	ft_box(t_parsing parse, t_box *box, t_camera cam, t_mlx *mlx)
{
	...
	while (axe.y < parse.size.y)
	{
		...
		box->posY = axe.y - parse.size.y / 2;
		box->posZ = 0.5 * parse.size.y;
		box->RowDis = box->posZ / box->posY;
		box->FStep.x = box->RowDis * (box->Rdir1.x - box->Rdir0.x)
			/ parse.size.x;
		box->FStep.y = box->RowDis * (box->Rdir1.y - box->Rdir0.y)
			/ parse.size.x;
...
// gestion separer du floor et celling pour texture de differente taille
